...
async function acaoLogin2(usuario, hashSenha) {
...
async function acaoLogin(usuario, hashSenha) {
  return new Promise(async (resolve, reject) => {
    setTimeout(() => {
      reject(new UtilsSS().gerarObjetoResposta(codigosAplicacao.erroNaoIdentificado, {}));
    }, 10000);
    objSessao = interfaceApp.abrirSessao("cadastro", usuario, hashSenha);
    saida = [objSessao];
    if (objSessao.estado == "aberta") {
...
function acaoGerarSenha(emailUsuario) {
  ...
  return new UtilsSS().gerarObjetoResposta(codigoSaida, {
    obj: objSaida,
  });
}

// (...[usuario, hashSenha, tokenTemporario]);
function acaoRedefinir(emailUsuario, hashNovaSenha, tokenTemporario) {
  // insere registro ou atualizda
  // se data estiver defasada, delete
  ...
  return new UtilsSS().gerarObjetoResposta(codigoSaida, objSaida);
}
