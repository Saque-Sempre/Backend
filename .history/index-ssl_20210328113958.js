...
function stringifyResponse(req) {
  return JSON.stringify(req.body);
}
...
function executarSQL(comando, nomeBD) {
  mensagemLog('info', '', '', `  > Executando SQL (executarSQL): ${comando.slice(0, 100)}`);
...
async function executarSQLPromise(comando, nomeBD) {
  ...
  mensagemLog('info', '', '', `  > Executando SQL (executarSQLPromise): ${comando.slice(0, 100)}`);
...
app.post("/", function (req, res) {
  ...
  argStr = JSON.stringify(req, getCircularReplacer());
