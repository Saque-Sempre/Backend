...
var app = express();
var diretorioLogServidor = "Server.log"
...
function mensagemLog(tipo, nivel, prefixo, mensagem) {
  tipoLog[tipo](nivel, String(new Date()) + ' - ' + prefixo, mensagem);
  fs.appendFile(diretorioLogServidor, `${tipo} - ${nivel} - ${String(new Date())} - ${prefixo} - ${mensagem}\n\n`, function (err) {
    if (err) return console.log(rr);
  });
}
...
var chavePrivada = fs.readFileSync('ChavesHTTPs/chavePrivada.key', 'utf8');
var certificado = fs.readFileSync('ChavesHTTPs/certificado.crt', 'utf8');
...
// Setup do Google Cloud. Para outras plataformas é diferente
var sqlConSetup = {
...
// Mensagens de retorno das requisicoes HTTP
...
function executarSQL(comando, nomeBD) {
  mensagemLog('info', '', '', `  > Executando SQL (executarSQL): ${comando.slice(0, 100)} - nomeBD: ${nomeBD} ...`);
...
async function executarSQLPromise(comando, nomeBD) {
  ...
  mensagemLog('info', '', '', `  > Executando SQL (executarSQLPromise): ${comando.slice(0, 100)} - nomeBD: ${nomeBD} ...`);
  var [regs] = await conexaoLocal.query(comando);
  return regs;
}
...
function novaInterfaceBD() {
  mensagemLog('info', '', '', `Invocando novaInterfaceBD()`);
...
class InterfaceApp {
  ...
  obterCredenciaisAditum(carteiraUsuarioSS) {
    ...
    var colunas = colunasCredenciaisAditum;
    ...
    mensagemLog('info', '', '', `Invocando obterCredenciaisAditum(${carteiraUsuarioSS})`);
  ...
  async obterTodasAsTransacoesBanco(carteiraUsuario, papelUsuario) {
    ...
    mensagemLog('info', '', '', `Invocando obterTodasAsTransacoesBanco(${carteiraUsuario}, ${papelUsuario})`);
  ...
  */
  ...
  abrirSessao(bd, usuario, hashSenha) {
    ...
    mensagemLog('info', '', '', `Invocando abrirSessao(${bd}, ${usuario}, ${hashSenha})`);
...
app.post("/login", function (req, res) {
  ...
  mensagemLog('info', '', '', `POST "/login"`);
  res = aplicarHeader(res);

  acaoLogin(usuario, hashSenha)
    .then((objSaida) => {
      mensagemLog('info', '', '', `POST "/login" ${usuario} ${hashSenha}`);
      mensagemLog('info', '', '', `POST "/login" ${usuario} ${hashSenha}: OK`);
      res.send(JSON.stringify(objSaida));
    })
    .catch((objExcecao) => {
      mensagemLog('error', '', '', `POST "/login" ${usuario} ${hashSenha}`);
      mensagemLog('info', '', '', `POST "/login" ${usuario} ${hashSenha}: ERRO`);
      res.send(JSON.stringify(objExcecao));
    });
});
...
app.post("/extrato", function (req, res) {
  ...
  mensagemLog('info', '', '', `POST "/extrato"`);
  res = aplicarHeader(res);

  acaoExtrato(usuario, hashSenha)
    .then((objSaida) => {
      mensagemLog('info', '', '', `POST "/extrato" ${usuario} ${hashSenha}`);
      mensagemLog('info', '', '', `POST "/extrato" ${usuario} ${hashSenha}: OK`);
      res.send(JSON.stringify(objSaida));
    })
    .catch((objExcecao) => {
      mensagemLog('error', '', '', `POST "/extrato" ${usuario} ${hashSenha}`);
      mensagemLog('info', '', '', `POST "/extrato" ${usuario} ${hashSenha}: ERRO`);
      res.send(JSON.stringify(objExcecao));
    });
});
...
app.post("/acessoAditum", function (req, res) {
  ...
  mensagemLog('info', '', '', `POST "/acessoAditum"`);
  res = aplicarHeader(res);

  acaoAcessoAditum(usuario, hashSenha)
    .then((objSaida) => {
      res.send(JSON.stringify(objSaida));
      mensagemLog('info', '', '', `POST "/acessoAditum" ${usuario} ${hashSenha}`);
      mensagemLog('info', '', '', `POST "/acessoAditum" ${usuario} ${hashSenha}: CORRETO`);
    })
    .catch((objExcecao) => {
      mensagemLog('error', '', '', `POST "/acessoAditum" ${usuario} ${hashSenha}`);
      mensagemLog('info', '', '', `POST "/acessoAditum" ${usuario} ${hashSenha}: ERRO`);
      res.send(JSON.stringify(objExcecao));
    });
});
...
/*httpsServer.listen(porta, () => {
  mensagemLog('info', '', '', `Servidor escutando na porta ${porta}!`);
  mensagemLog('info', '', '', `O servidor usa SSL!\n`);
});*/

app.listen(porta, function () {
  mensagemLog('info', '', '', `Servidor escutando na porta ${porta}!`);
  mensagemLog('info', '', '', String(new Date()));
  mensagemLog('warn', '', '', `Usa SSL: NÃO`);
  mensagemLog('verbose', '', '', `\n`);
});
