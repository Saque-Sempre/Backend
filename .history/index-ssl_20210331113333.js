...
function executarSQL(comando, nomeBD) {
  mensagemLog('info', '', '', `  > Executando SQL (executarSQL): ${comando.slice(0, 10000)} - nomeBD: ${nomeBD} ...`);
...
async function executarSQLPromise(comando, nomeBD) {
  ...
  mensagemLog('info', '', '', `  > Executando SQL (executarSQLPromise): ${comando.slice(0, 10000)} - nomeBD: ${nomeBD} ...`);
...
app.post("/", function (req, res) {
  ...
  if (acao in tratadorRota) {
    tratadorRota[acao](...[usuario, hashSenha, tokenTemporario])
      .then((objSaida) => {
        mensagemLog('info', '', '', `POST "/" ${argStr} : OK`);
        res.send(JSON.stringify(objSaida));
      })
      .catch((objExcecao) => {
        mensagemLog('error', '', '', `POST "/" ${argStr} : ERRO`);
...
app.post("/login", function (req, res) {
  ...
  acaoLogin(usuario, hashSenha)
    ...
    .catch((objExcecao) => {
      mensagemLog('error', '', '', `POST "/login" ${usuario} ${hashSenha}: ERRO`);
      res.send(JSON.stringify(objExcecao));
    });
});
...
app.post("/extrato", function (req, res) {
  ...
  acaoExtrato(usuario, hashSenha)
    ...
    .catch((objExcecao) => {
      mensagemLog('error', '', '', `POST "/extrato" ${usuario} ${hashSenha}: ERRO`);
      res.send(JSON.stringify(objExcecao));
    });
});
...
app.post("/acessoAditum", function (req, res) {
  ...
  acaoAcessoAditum(usuario, hashSenha)
    ...
    .catch((objExcecao) => {
      mensagemLog('erro', '', '', `POST "/acessoAditum" ${usuario} ${hashSenha}: ERRO`);
      res.send(JSON.stringify(objExcecao));
    });
});
