...
const consoleErro = (req) => {
  console.error("> " + req.method + " - " + req.url + " - " + String(new Date()));
};
...
app.post("/", function (req, res) {
  ...
  if (acao in tratadorRota) {
    tratadorRota[acao](...[usuario, hashSenha, tokenTemporario])
      .then((objSaida) => {
        consoleRequisicao("/: Sucesso");
        res.send(JSON.stringify(objSaida));
      })
      .catch((objExcecao) => {
        consoleRequisicao("/: Excecao");
...
app.post("/login", function (req, res) {
  ...
  consoleRequisicao("LOGIN: " + String(req));

  res = aplicarHeader(res);

  acaoLogin(usuario, hashSenha)
    .then((objSaida) => {
      consoleRequisicao("\\login: Sucesso");
      res.send(JSON.stringify(objSaida));
    })
    .catch((objExcecao) => {
      consoleRequisicao("/extrato: Excecao");
      res.send(JSON.stringify(objExcecao));
    });
});
...
app.post("/extrato", function (req, res) {
  ...
  acaoExtrato(usuario, hashSenha)
    .then((objSaida) => {
      consoleRequisicao("/extrato: Sucesso");
      res.send(JSON.stringify(objSaida));
    })
    .catch((objExcecao) => {
      consoleRequisicao("/extrato: Excecao");
...
app.post("/acessoAditum", function (req, res) {
  ...
  acaoAcessoAditum(usuario, hashSenha)
    .then((objSaida) => {
      consoleRequisicao("/acessoAditum: Sucesso");
      res.send(JSON.stringify(objSaida));
    })
    .catch((objExcecao) => {
      consoleRequisicao("/acessoAditum: Excecao");
      res.send(JSON.stringify(objExcecao));
    });
});
